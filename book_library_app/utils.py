...
def token_required(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            print("1token work", token)
            playload = jwt.decode(token, current_app.config.get('SECRET_KEY'), algorithms=["HS256"])
            print("2token work", token)

        except jwt.ExpiredSignatureError as e:
            print("ExpiredSignatureError: ", e)
            abort(401, description='Expired token. Please to get new token')
        except jwt.InvalidTokenError as e:
            print("InvalidTokenError: ", e)
            abort(401, description='Invalid token. Please login or register')
        else:
            return func(playload['user_id'], *args, **kwargs)
    return wrapper
